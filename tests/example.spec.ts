...
test.describe('End-to-end Signing Flow', () => {
  test('User can create a document from a template and view it', async ({ page }) => {
    ...
    const useTemplateButton = page.locator('table > tbody > tr').first().getByRole('button', { name: 'Use Template' });
    await expect(useTemplateButton).toBeVisible();
    await useTemplateButton.click();

    // The dialog for naming the document should appear.
    await expect(page.getByRole('heading', { name: 'Name Your Document' })).toBeVisible();
