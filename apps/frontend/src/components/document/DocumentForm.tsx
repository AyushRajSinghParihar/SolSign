...
export function DocumentForm({ document: initialDocument, template }: DocumentFormProps) {
  ...
  const [formState, setFormState] = useState<FormState>(document.filled_data_json || {})
  ...
  }, [getVaultItemsQuery.data, signMessage, template.id, document.filled_data_json])
  ...
  const handleSaveDraft = async () => {
    toast.promise(saveMutation.mutateAsync({ documentId: document.id, filledData: formState }), {
  ...
  const handleSignDocument = async () => {
    ...
    await saveMutation.mutateAsync({ documentId: document.id, filledData: formState })

    // 2. Create a stable JSON string and hash it
    const documentJson = JSON.stringify(formState, Object.keys(formState).sort())
    ...
    const promise = signMutation.mutateAsync({ documentId: document.id, documentHash })
